"use client";
// src/components/layout/ProductPageClient.tsx
...
const Breadcrumb = styled.nav`
  ...
  span { color: ${({ theme }) => theme.colors.border2}; }
`;
...
const MTagRow = styled.div`display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: .75rem;`;
...
const MHr = styled.hr`
  border: none; border-top: 1px solid ${({ theme }) => theme.colors.border}; margin: 1.25rem 0;
`;

const MActions = styled.div`
  display: flex; gap: 8px; flex-direction: column;
  ${up.sm} { flex-direction: row; flex-wrap: wrap; }
`;
...
export default function ProductPageClient({ store, product, slug }: Props) {
      ...
      <DesktopWrap>
        <DHeader>
          <DHeaderInner>
            ...
            <Divider />
            <StoreLink href={`/${slug}`}>{store.name}</StoreLink>
          </DHeaderInner>
        </DHeader>

        <DContent>
          <Breadcrumb>
            <Link href={`/${slug}`}>{store.name}</Link>
            <span>/</span>
